...
opt = {
   ...
   checkpoints_dir = './checkpoints/OP', -- models are saved here
   cudnn = 1,                         -- set to 0 to not use cudnn
   condition_GAN = 1,                 -- set to 0 to use unconditional discriminator
   use_GAN = 1,                       -- set to 0 to turn off GAN term
   use_L_term = 1,                    -- set to 0 to turn off L term, 1 to use L1 term, 2 to use L2 term
   which_model_netD = 'basic', -- selects model to use for netD, default:basic
   which_model_netG = 'unet',  -- selects model to use for netG
   n_layers_D = 1,             -- only used if which_model_netD=='n_layers'
   lambda = 100,               -- weight on L1 term in objective, default:100
}
...
function defineG(input_nc, output_nc, ngf)
    ...
    elseif opt.which_model_netG == "resnet" then netG = defineG_resnet(input_nc, output_nc, ngf)
    elseif opt.which_model_netG == "unet_cadd" then netG = defineG_unet_cadd(input_nc, output_nc, ngf)
    elseif opt.which_model_netG == "unet_double" then netG = defineG_unet_double(input_nc, output_nc, ngf)
...
function defineD(input_nc, output_nc, ndf)
    ...
    elseif opt.which_model_netD == "one" then netD = defineD_one(input_nc_tmp, output_nc)
...
if opt.gpu > 0 then
   ...
   netD:cuda(); netG:cuda(); criterion:cuda(); criterionAE:cuda(); criterion_mse:cuda();
   print('done')
else
...
local fGx = function(x)
    ...
    if opt.use_L_term==1 then
       errL1 = criterionAE:forward(fake_B, real_B)
       df_do_AE = criterionAE:backward(fake_B, real_B)
    elseif opt.use_L_term==2 then
       errL1 = criterion_mse:forward(fake_B, real_B)
       df_do_AE = criterion_mse:backward(fake_B, real_B)
    else
        errL1 = 0
    end
